...
import { ADDEMAILTES, AddPatientapi, GEtEmailSingle, GEtSMSSingle, UPdateEmailTemplate, UPdateSms } from '@components/Api/AllApi';
...
const EditSMSTempalets = ({ type }) => {
    ...
    const { id, name,  content } = pro;
    console.log(id, name,  content)
    useEffect(() => {

        const EmailData = GEtSMSSingle(p_id)
        if (EmailData) {
            EmailData.then((data) => {
    ...
    const handleUpdate = (e) => {
        ...
        try {
            console.log(id,name,content, "qqqqqqqqqqqqqqqqqqqqqqqq")
            const result = UPdateSms(id,name,content);
...
EditSMSTempalets.propTypes = {
    type: PropTypes.oneOf(['patient']).isRequired
}

export default EditSMSTempalets;